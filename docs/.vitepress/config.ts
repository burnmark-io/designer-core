...
export default defineConfig({
  ...
  lastUpdated: true,
  cleanUrls: true,
  // Everything the link checker *can* validate is strict. Two carve-outs:
  //   - `/reference/api/**` is generated by typedoc at build time into a
  //     gitignored directory; VitePress's static check runs before that
  //     output is discovered.
  //   - `/assets/*.label` are raw JSON files served from `docs/assets/`,
  //     not markdown pages, so the link checker has no way to verify them.
  // Anything else (broken internal markdown link, typo in a /guide/ path)
  // still fails the build.
  ignoreDeadLinks: [/^\/reference\/api(\/.*)?$/, /\.label$/],
  themeConfig: {
    nav: [
