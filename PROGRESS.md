...
- [x] Flipped `ignoreDeadLinks` back off except for two narrow regex
      carve-outs: `/reference/api/**` (generated by typedoc at deploy
      time, gitignored) and `*.label` (raw JSON assets, not markdown
      pages). Any other broken internal link now fails the build.
- [x] Ran `docs:build` with strict checking — passes cleanly
- [x] Cross-checked every symbol imported from
      `@burnmark-io/designer-core`, `@burnmark-io/designer-vue`, and
      `@burnmark-io/designer-react` against the shipped public exports.
      No missing symbols.
- [x] Synced `guide/template-engine.md` and `reference/label-format.md`
      with the live `docs/assets/christmas-card.label` — the recipient
      name object now renders on the red plane (`#ff0000`) in all
      three places, matching the committed `.label` file. Template-
      engine walkthrough updated to pass `TWO_COLOR_BLACK_RED` into
      `renderBatch` so the two-colour design actually prints in two
      colours.
- [x] Added `.claude/` to `.gitignore` so local agent state stays out
      of commits.
- [x] Deploy: `docs.yml` GitHub Actions workflow runs
      `pnpm docs:api && pnpm docs:build` on every push to `main` and
      publishes to GitHub Pages at
      `https://burnmark-io.github.io/designer-core/` — no manual
      deploy step needed.
